...
var cacheList = [
	...
	"resource/assets/common/json/locale-7a41c566a6.bgjson",
	...
	"resource/js/res_L-b41375bf48.bgjson",
	"resource/js/thm-3e98986ba0.js",
		...
		cdnRes.map(async url => {
			...
			try {
				...
				if (response && !bSuccess) {
					...
					if (data) {
						// 設置url             
						console.warn("serviceWorker sw set cdn", url, version);
						curCdn = url;
						bSuccess = true;
						resolve(url);
						return;
				...
				// 忽略錯誤文件
				console.warn("serviceWorker sw get cdn fail", url, error.toString(), version);
			}
			if (++failCount == total) {
				reject("all fail");
			}
...
self.addEventListener('activate', function (event) {
	async function deleteCurCache() {
		...
		console.warn("serviceWorker sw deleteCurCache complete", version);
	}
	const doDeleteJob = async () => {
		await Promise.all([deleteCurCache()])
			.then(function () {
				console.warn("serviceWorker sw activate complete", version);
			})
	}
