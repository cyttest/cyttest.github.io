let cdnRes=["https://cdnx-beta.hswenfa.com"];const self=this,isPwa=!0,checkHash="a603d8de9b",version="4.0.8.27",swName="[sw_pwa]",openName="pwa",idx=self.location.pathname.lastIndexOf("/"),URL_DOMAIN=self.location.origin,URL_PATH=self.location.pathname.substring(0,idx+1),INDEX_HTML=URL_DOMAIN+URL_PATH+"index.html";console.warn(swName+" init URL_DOMAIN",URL_DOMAIN,version),console.warn(swName+" init URL_PATH",URL_PATH,version);let htmlVersion,replaceCdn,gamePath="/h5V01/m/";{const e=async()=>{console.warn(swName+" checkCdn!",version);let e=0,t=!1;const n=cdnRes.length;return new Promise(((a,s)=>{cdnRes.map((async i=>{const o=i+gamePath+"config.json?v="+Math.floor(1e4*Math.random());try{const e=await fetch(o);if(e&&e.ok&&!t){if(await e.json())return console.warn(swName+" set cdn",i,version),replaceCdn=i,t=!0,void a(i)}}catch(e){console.warn(swName+" get cdn fail",i,e.toString(),version)}++e==n&&s("all fail")}))}))},t=async()=>{console.warn(swName+" startCheck",version),bCheckingCdn=!0;try{await e(),bCheckingCdn=!1,console.warn(swName+" checkCdn end",version)}catch(e){console.warn(swName+" checkCdn fail",e.toString(),version),setTimeout(t,3e3)}};t()}function checkUrl(e){return e=new URL(e).pathname.replace(URL_PATH,""),cacheList.includes(e)}async function putInCache(e,t){let n=e.url;if(200==t.status){let a=t.clone();(await caches.open(openName)).put(e,a),console.warn(swName+" putInCache success",n,version)}else console.warn(swName+" putInCache no ",n,t.status,version)}async function handleRangeFile(e,t){const n=await t.arrayBuffer(),a=/^bytes\=(\d+)\-(\d+)?$/g.exec(e.headers.get("range"));if(a){const e=Number(a[1]),t=Number(a[2])||n.byteLength-1;return new Response(n.slice(e,t+1),{status:206,statusText:"Partial Content",headers:[["Content-Range",`bytes ${e}-${t}/${n.byteLength}`]]})}return new Response(null,{status:416,statusText:"Range Not Satisfiable",headers:[["Content-Range",`*/${n.byteLength}`]]})}async function fetchFile(e,t){let n,a,s=e.origin+e.pathname,i=!1;try{if(s==INDEX_HTML)n=new Request(s,{cache:"reload"});else{if(i=checkUrl(s),i&&(a=await caches.match(t)),a)return t.headers.get("range")?handleRangeFile(t,a):a;{let a={mode:"cors",credentials:"omit",referrerPolicy:"",headers:new Headers};if(replaceCdn){let t=s.replace(e.origin,replaceCdn);t=t.replace(URL_PATH,gamePath),n=new Request(t,a)}else n=new Request(t,a)}}const o=()=>new Promise(((e,a)=>{let o=!1;s==INDEX_HTML&&setTimeout((()=>{o||a("timeout")}),5e3),fetch(n).then((n=>{n.ok?(o=!0,i&&putInCache(t,n),e(n)):a("status="+n.status)})).catch((e=>{console.warn(swName+" fetch error",t,e.toString(),version),a("error="+e.toString())}))}));a=await o()}catch(e){}return a||(a=s==INDEX_HTML?new Response(OFFLINE_HTML,{headers:{"Content-Type":"text/html"}}):new Response("Network error happened",{status:404,headers:{"Content-Type":"text/plain"}})),a}async function checkSW(e){try{let t=await fetch(self.location.href,{cache:"reload"});if(e&&t&&t.ok){let e=(await t.clone().text()).match(/checkHash\s*=\s*"([^"]*)"/);if(console.warn(swName+"sw_update  checkHash",e[1],version),e&&e[1]!=checkHash)callClients("sw_update","sw version diff!");else if(htmlVersion&&htmlVersion!=version){let e=await fetch(INDEX_URL,{cache:"reload"});if(e&&e.ok){let t=(await e.clone().text()).match(/htmlInfo.version\s*=\s*"([^"]*)"/);console.warn(swName+"sw_update  checkHtml",t[1],version),t&&t[1]==version&&callClients("sw_reload","html:"+htmlVersion+",sw:"+version)}}}}catch(e){console.warn(swName+"  checkSW fail",e.toString(),version)}}function callClients(e,t){self.clients.matchAll().then((n=>{console.warn(swName+" callClients ",e,t,n,version),n.forEach((n=>{n.postMessage({type:e,value:t})}))}))}self.addEventListener("install",(function(e){console.warn(swName+" install!",version);e.waitUntil((async()=>{console.warn(swName+" install skipWaiting!",version),self.skipWaiting()})())})),self.addEventListener("activate",(function(e){async function t(){const e=await caches.open(openName),t=await e.keys();await Promise.all(t.map((function(t){let n=t.url;if(0==checkUrl(n))return console.warn(swName+" deleteCurCache delete",n,version),e.delete(t)}))),console.warn(swName+" deleteCurCache complete",version)}console.warn(swName+" activate",version),self.clients.claim(),e.waitUntil((async()=>{await Promise.all([t()]).then((function(){console.warn(swName+" activate complete",version)}))})())})),self.addEventListener("message",(e=>{console.warn(swName+" message ",e.data,version),e.data&&("VERSION"===e.data.type?(htmlVersion=e.data.value,htmlVersion!=version&&checkSW(!0)):"UPDATE"===e.data.type&&checkSW(e.data.value||htmlVersion!=version))})),self.addEventListener("fetch",(function(e){let t=e.request,n=t.url;if("GET"!==t.method)return!1;if(n.match(/\?/)&&(n=n.split("?")[0]),n.match(/pwa_sw.js|pwa_dir|pwa.html|index_a.html/))return!1;let a=new URL(n),s=a.origin.replace(/-[\d.]+/,".")+a.pathname;return("navigate"!=t.mode||n==INDEX_HTML)&&(!!s.match(URL_DOMAIN+URL_PATH)&&void e.respondWith(fetchFile(a,t)))}));const OFFLINE_HTML='<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title></title><style>a,body,button,div,h1,h2,html,p,video{margin:0;padding:0;border:0;font-size:100%;font:inherit;vertical-align:baseline}body{text-align:center;font-family:Arial,sans-serif}.main{width:100%;height:100%;overflow:auto;position:fixed}.box{width:90%;max-width:500px;position:absolute;box-sizing:border-box;padding:20px 0;left:50%;transform:translateX(-50%);-webkit-transform:translateX(-50%)}@media screen and (min-height:330px){.box{transform:translateX(-50%) translateY(-50%);-webkit-transform:translateX(-50%) translateY(-50%);top:50%;left:50%}}#offline-message{color:#676767;font-size:20px;letter-spacing:.1rem;box-sizing:border-box;padding-bottom:20px}#offline-message h1{font-size:30px;box-sizing:border-box;padding-bottom:10px}#svgImg svg{color:#ccc;fill:currentColor;stroke:#ccc;height:calc(35vh);min-height:65px;max-height:200px}#reload-button{padding:10px;font-size:18px;cursor:pointer;background-color:#4b72e2;color:#fff;border:none;max-width:400px;width:90%;font-weight:500}#tipsBox{padding-top:10px}#tips{color:#5b7778;font-size:18px}#tips:hover{color:#1ab394}</style></head><body><div class="main"><div class="box"><div id="svgImg"><svg viewBox="0 0 512 512" xmlns="http://www.w3.org/2000/svg"><title>ionicons-v5-s</title><path d="M93.72,183.25C49.49,198.05,16,233.1,16,288c0,66,54,112,120,112H320.37" style="fill:none;stroke-linecap:round;stroke-linejoin:round;stroke-width:32px"/><path d="M467.82,377.74C485.24,363.3,496,341.61,496,312c0-59.82-53-85.76-96-88-8.89-89.54-71-144-144-144-26.16,0-48.79,6.93-67.6,18.14" style="fill:none;stroke-linecap:round;stroke-linejoin:round;stroke-width:32px"/><line x1="448" y1="448" x2="64" y2="64" style="fill:none;stroke-linecap:round;stroke-miterlimit:10;stroke-width:32px"/></svg></div><div class="other"><div id="offline-message"><h1 id="title"></h1><p id="content"></p></div><button id="reload-button" onclick="reloadPage()"></button><div id="tipsBox"><a id="tips" href="javascript:;" target="_blank">123</a></div></div></div></div><script>var args={},str=location.search.split("?")[1];if(str)for(var seg=str.split("&"),i=seg.length-1;0<=i;i--)if(seg[i]){var s=seg[i].split("=");args[s[0]]=s[1]}var strObj={title:"OFFLINE",content:"Please check your internet connect",retry:"Retry",tips:"Back"};args.locale&&(args.locale=args.locale.replace(/-/g,"_"),"zh_CN"==args.locale&&(strObj={title:"网路异常",content:"请检查您的网路连接。",retry:"重整页面",tips:"返回"}));var title=document.getElementById("title");title.innerText=document.title=strObj.title;var content=document.getElementById("content");content.innerText=strObj.content;var reloadBtn=document.getElementById("reload-button");reloadBtn.innerText=strObj.retry;var tipsBox=document.getElementById("tipsBox");if(tipsBox.style.display="none",args.referrer){var pwa="true"==args.pwaMode||navigator.standalone||window.matchMedia("(display-mode: standalone)").matches||window.matchMedia("(display-mode: fullscreen)").matches,mode=args.mode?parseInt(args.mode):0,cui=args.cui?parseInt(args.cui):0,bFromAPP=!pwa&&(256==(256&mode)||64==(64&cui));if(!bFromAPP){tipsBox.style.display="block";var tips=document.getElementById("tips");tips.href=args.referrer,tips.innerText=strObj.tips,tips.target=pwa?"_blank":"_top"}}function reloadPage(){location.reload()}<\/script></body></html>',cacheList = [
	...
	"resource/js/res_L-59180b51dd.bgjson",
	...
	"resource/assets/login/mx15/login_bg_mx15_l-4beb725a2e.bgjpg",
	"resource/assets/login/mx15/login_bg_mx15_p-4733629865.bgjpg",
	"resource/assets/login/mx15/login_logo_mx15-d13ef20fac.bgpng",
